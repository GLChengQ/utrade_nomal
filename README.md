...
- **MySQL 持久化**：成交历史、平仓类型、账户重置事件同步到本地 MySQL，面板可查历史
...
`BOT_STRATEGY` 选择策略，默认 `vgas`。三者共用 ATR 止损与同一套风控；VGAS/海龟**不设固定止盈**（利润由移动止损锁定），EMA 保留固定止盈。
...
- **离场**：反向快速通道(`BOT_EXIT_PERIOD`=10)突破 或 2×ATR 止损；**不设固定止盈**，利润由移动止损（峰值 - 2×ATR）锁定
...
| 交易所原生止盈 | EMA 策略进场挂 `TAKE_PROFIT_MARKET`；VGAS/海龟不设固定止盈，靠移动止损锁定利润 | — |
...
日亏熔断会在 `BOT_HALT_COOLDOWN_HOURS`(8 小时)冷却后自动恢复（并重设当日基准）；最大回撤熔断可通过面板「恢复交易」手动解除（写入 `state/resume.flag`，重新基线）。熔断整体也可用面板「熔断开关」临时关闭（`state/breaker_on.flag`，测试环境默认关闭）。
...
| `BOT_HALT_COOLDOWN_HOURS` | `8.0` | 日亏熔断后自动恢复的小时数 |
...
| `MYSQL_USER` | `root` | 用户名 |
| `MYSQL_PASSWORD` | `` | 密码（本地 root 默认空） |
| `MYSQL_DATABASE` | `binance_bot` | 数据库名（不存在则自动创建） |
| `RESET_DETECT_PCT` | `20.0` | 钱包骤降百分比阈值，判定账户重置 |

> MySQL 是**可选**的：连不上数据库时 bot 照常交易，只跳过历史落库。
...
系统会把每次平仓和账户重置事件同步到本地 MySQL（`pymysql`，可选）：

- **自动建库建表**：数据库 `binance_bot`（可配），表 `trades`（成交历史）、`resets`（账户重置）、`circuit_logs`（熔断日志）、`daily_summary`（每日盈亏汇总）、`equity_snapshots`（权益快照）。
- **成交落库**：每笔平仓记录交易对、方向、数量、仓位单元、开仓/止损/止盈/平仓价、盈亏、保证金、加仓明细、平仓原因与**平仓类型**。
- **平仓类型识别**：止损 / 止盈 / 主动平仓 / 信号平仓（反向信号、离场信号）。
- **账户重置检测**：测试网钱包余额单次骤降 ≥ `RESET_DETECT_PCT`(20%) 时记录一条重置事件并告警（测试网会定期清零）。
- **每日盈亏汇总**：`daily_summary` 按日汇总已实现盈亏、重置影响、净权益与熔断次数，供面板日历展示。
- **净权益曲线**：5000 起步 + 累计已实现盈亏（剔除测试网重置影响），与真实权益曲线对照。
- **北京时间**：落库时间统一转为北京时间（UTC+8）。
- **不阻断交易**：数据库不可用时仅告警并跳过落库，交易照常运行。
- 面板新增 `/api/history`、`/api/calendar`、`/api/curves` 接口。

```powershell
# 默认连本地 root@127.0.0.1:3306（空密码），需自定义时在 .env 配置 MYSQL_* 变量
```

## 可视化面板（H5，手机可看）

内置轻量 H5 面板（`dashboard/`），展示：账户权益、可用余额、未实现盈亏、权益/净权益曲线、
实时持仓（带**手动平仓**按钮）、最近成交、系统状态（时段/熔断/持仓数）、**每日盈亏日历**、**熔断日志**，
以及从 MySQL 读取的**历史成交**与**账户重置记录**；熔断时提供**恢复交易**按钮（含倒计时）与**熔断开关**。
