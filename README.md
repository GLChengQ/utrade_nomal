...
日亏熔断会在 `BOT_HALT_COOLDOWN_HOURS`(8 小时)冷却后自动恢复（并重设当日基准）；最大回撤熔断需手动解除。
...
- **自动建库建表**：数据库 `binance_bot`（可配），表 `trades`（成交历史）、`resets`（账户重置记录）。
- **成交落库**：每笔平仓记录交易对、方向、数量、仓位单元、开仓/止损/止盈/平仓价、盈亏、平仓原因与**平仓类型**。
- **平仓类型识别**：止损 / 止盈 / 主动平仓 / 信号平仓（反向信号、离场信号）。
- **账户重置检测**：测试网钱包余额单次骤降 ≥ `RESET_DETECT_PCT`(20%) 时记录一条重置事件并告警（测试网会定期清零）。
- **不阻断交易**：数据库不可用时仅告警并跳过落库，交易照常运行。
- 面板新增 `/api/history` 接口，展示 MySQL 历史成交与重置记录。
...
实时持仓、最近成交、系统状态（时段/熔断/持仓数），以及从 MySQL 读取的**历史成交**与**账户重置记录**。
